...
<<<<<<< HEAD
    ...
    try:
        # Try saving using XGBoost's native format first
        if hasattr(model, "_Booster"):
            model._Booster.save_model(filepath + ".json")
            logger.info(f"Model saved in XGBoost native format to {filepath}.json")
        else:
            # Fallback to pickle if not an XGBoost model
            with open(filepath, 'wb') as f:
                pickle.dump(model, f)
            logger.info(f"Model saved in pickle format to {filepath}")
    except Exception as e:
        logger.warning(f"Failed to save in native format: {e}, falling back to pickle")
        with open(filepath, 'wb') as f:
            pickle.dump(model, f)
        logger.info(f"Model saved in pickle format to {filepath}")
=======
...
<<<<<<< HEAD
    if os.path.exists(filepath + ".json"):
        ...
        try:
            ...
            model.load_model(filepath + ".json")
            logger.info(f"Model loaded from {filepath}.json")
            return model
        except Exception as e:
